...
class Exponential(Schedule):
    ...
    Arguments:
        initial_value (float): the initial value at ``step=1``.
        final_value (float): the value at ``step=num_steps``.
        num_steps (int, optional): the total number of steps over which the value will be scheduled.
    ...
    """
    def __init__(self, initial_value, final_value, num_steps=2, stop_factor=None, loss_key='training_loss'):
